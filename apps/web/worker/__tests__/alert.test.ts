...
const event: AlertEvent = {
  ...
  links: [{ label: "open", url: "https://aidr.today/llm/abc" }],
...
describe("AlertEvent adapters", () => {
  ...
  it("escapes HTML in the Telegram payload", () => {
    ...
    expect(html).toContain('href="https://aidr.today/llm/abc"');
  ...
  it("escapes quotes inside Telegram href attributes", () => {
    ...
    expect(html).toContain(
      'href="https://aidr.today/x?q=&quot;quoted&quot;&amp;s=&#39;s&#39;"'
    );
    expect(html).toContain("say &quot;hi&quot;");
    expect(html).not.toContain('href="https://aidr.today/x?q="');
  });
...
describe("webhookDeliveryId", () => {
  ...
  it("uses Slack payloads only for hooks.slack.com", async () => {
    ...
    expect(jsonBody.source).toBe("aidr.today");
    vi.unstubAllGlobals();
  });
});
